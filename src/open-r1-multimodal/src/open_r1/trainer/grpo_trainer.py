...
class RepeatRandomSampler(Sampler):
    ...
    def __init__(
        self,
        data_source: Sized,
        mini_repeat_count: int,  #num_generations 4
        batch_size: int = 1,
        repeat_count: int = 1,
...
class Qwen2VLGRPOTrainer(Trainer):
    ...
    def __init__(
        ...
        if is_deepspeed_zero3_enabled():
            if "Qwen2-VL" in model_id:
                ...
                # self.ref_model = AutoModelForCausalLM.from_pretrained(model_id, **model_init_kwargs)  #9.5
                self.ref_model = Qwen2_5_VLForConditionalGeneration.from_pretrained(model_id, **model_init_kwargs)
    ...
    def _generate_and_score_completions(self, inputs: dict[str, Union[torch.Tensor, Any]], model) -> dict[str, Union[torch.Tensor, Any]]:
        ...
        #     if diff > 0:
        #         prompt_inputs["attention_mask"] = attention_mask[..., :input_ids.shape[-1]]
        #     elif diff < 0:
        #         pad = torch.zeros(
        #             attention_mask.shape[:-1] + (abs(diff),),
        #             dtype=attention_mask.dtype, device=attention_mask.device
        #         )
        #         prompt_inputs["attention_mask"] = torch.cat([pad, attention_mask], dim=-1)
        # #到此
    ...
    def _get_train_sampler(self) -> Sampler:
        ...
        return RepeatRandomSampler(
            ...
            batch_size=effective_batch_size // self.num_generations,  #effective_batch_size = 1 * 4 * 2 = 8
