...
def result_reward(completions, solution, **kwargs):
    ...
    MAX_INCREASE = float(os.getenv("MAX_INCREASE", "0.60"))
    ...
    for content, sol in zip(contents, solution):
        ...
        base_w = {
            "in": 0.35,      # initial in_reward weight
            "region": 0.35,
            "area": 0.30
        }
        w_in = min(base_w["in"] + increase, 0.98)
        remaining = max(1.0 - w_in, 0.0)
        others_base_sum = base_w["region"] + base_w["area"]
        if others_base_sum <= 0:
            w_region = w_area = remaining / 2.0 if remaining > 0 else 0.0
        else:
            w_region = base_w["region"] / others_base_sum * remaining
            w_area = base_w["area"] / others_base_sum * remaining

        region_component = region_reward * iou_reward
        in_component = in_reward
        area_component = area_reward

        reward = w_region * region_component + w_in * in_component + w_area * area_component
            ...
            try:
                with open(log_path, "a", encoding="utf-8") as f:
                    f.write(
                        ...
                        f"increments={increments} w_in={w_in:.4f} w_region={w_region:.4f} w_area={w_area:.4f} "
                        f"components: region_comp={region_component:.4f}, in_comp={in_component:.4f}, area_comp={area_component:.4f}\n"
                    )
